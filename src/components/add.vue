...
<!-- type2 -->
    <div v-if="moduleType.type==2" class="item2" ref='item2'>
      <div class="item2_border">
          ...
          <div class="item2_item" v-for="(ele,i) in moduleType.list" :key=i >
                ...
                <div v-if=ele.type&&ele.list>
                  <add  v-for="(child,index) in ele.list" :key=index :moduleType=child :dataArr=ele.child :index=index></add>
                </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  created(){
   ...
   mounted(){
     if(this.dataArr){
      console.log('ref',this.$refs.item2.style.width,this.dataArr[this.index].list)
